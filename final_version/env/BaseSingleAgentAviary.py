...
from env.BaseAviary import BaseAviary
...
class BaseSingleAgentAviary(BaseAviary):
    ...
    #        return np.array(self.HOVER_RPM + action * 0.03 * self.HOVER_RPM)
        
        elif self.ACT_TYPE == ActionType.RPM:

            # =============================
            # SMOOTH HOVER CONTROLLER
            # =============================
            ...
            # ---------- HEIGHT CONTROL ----------
            z_error = target[2] - pos[2]

            #  limit error (prevents jump)
            z_error = np.clip(z_error, -0.5, 0.5)

            #  smooth control (no bounce)
            z_correction = 20 * z_error - 110 * vel[2]

            base = self.HOVER_RPM + z_correction

            #  LIMIT THRUST (VERY IMPORTANT)
            base = np.clip(base, self.HOVER_RPM - 2000, self.HOVER_RPM + 2000)

            # ---------- STABILIZATION ----------
            roll_corr  = -200 * rpy[0] - 20 * ang_vel[0]
            pitch_corr = -200 * rpy[1] - 20 * ang_vel[1]
            yaw_corr   = -30  * rpy[2] - 10 * ang_vel[2]

            rpm = np.array([
                base + roll_corr - pitch_corr + yaw_corr,
                base - roll_corr - pitch_corr - yaw_corr,
                base - roll_corr + pitch_corr + yaw_corr,
                base + roll_corr + pitch_corr - yaw_corr
            ])
