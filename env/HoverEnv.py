...
class HoverEnv(BaseSingleAgentAviary):
    ...
    def _addObstacles(self):
        ...
        if scenario == "slalom":
            ...
            # Simplest possible gates: Just two tall static poles on the ground (like ski slalom)
            g_width = 0.6  # 1.2m gap total
            for x in [1.5, 3.0, 4.5]:
                color = [0.8, 0.3, 0.1, 1.0] # Matte safety orange
                # Left pole touching ground (height is 2.0m, center is at Z=1.0)
                self.obstacle_configs.append({
                    "pos": np.array([x, -g_width, 1.0], dtype=np.float32), 
                    "axis": 1, "speed": 0.0, "bounds": [0,0], "direction": 1.0, 
                    "shape": "cylinder", "radius": 0.05, "color": color
                })
                # Right pole touching ground
                self.obstacle_configs.append({
                    "pos": np.array([x, g_width, 1.0], dtype=np.float32), 
                    "axis": 1, "speed": 0.0, "bounds": [0,0], "direction": 1.0, 
                    "shape": "cylinder", "radius": 0.05, "color": color
